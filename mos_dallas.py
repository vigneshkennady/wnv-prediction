...
def fetch_ca_wnv_year(year: int, download_id: int) -> Optional[pd.DataFrame]:
    ...
    if "county" in df.columns:
        df = df[df["county"].str.strip().str.upper() == "LOS ANGELES"]
    if df.empty:
        return None
...
ACCESS_CHECKS: dict = {
    ...
    "dshs_tx": {
        "label":  "Texas DSHS — Dallas County TX (PDF report index)",
...
def main() -> None:
    ...
    # ── Source E: Dallas / DSHS PDFs ──────────────────────────────────────
    if access.get("dshs_tx"):
        log.info("\n[Step 5] Dallas County TX — DSHS PDF reports")
        df_dallas = fetch_dallas_dshs()
        if not df_dallas.empty: all_agg.append(df_dallas)
    else:
        log.info("\n[Step 5] Dallas County TX — DSHS not reachable; trying manual files")
        manual_dir = Path("./manual_data/dallas_tx")
        if manual_dir.exists():
            df_dallas = fetch_dallas_dshs(start_year=END_YEAR + 1)  # skip PDF download
            if not df_dallas.empty: all_agg.append(df_dallas)
    ...
    if all_agg:
        ...
        out = OUTPUT_DIR / "mosquito_surveillance_county_week.csv"
        df_out.to_csv(out, index=False)
        log.info("  -> County-week aggregate : %d rows x %d cols  -> %s",
                 len(df_out), len(df_out.columns), out)
    else:
        log.warning("  No aggregate data collected across any source")

    if all_pools:
        df_pools_out = pd.concat(all_pools, ignore_index=True)
        out = OUTPUT_DIR / "mosquito_surveillance_pool_level.csv"
        df_pools_out.to_csv(out, index=False)
        log.info("  -> Pool-level records    : %d rows x %d cols  -> %s",
                 len(df_pools_out), len(df_pools_out.columns), out)

    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 65)
    print("Output files:")
    for f in sorted(OUTPUT_DIR.glob("mosquito_surveillance*.csv")):
